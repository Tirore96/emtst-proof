...
Require Import SendRec.Nom_SyntaxBST.
...
Check env. Locate ordType. Print atom_ordType.
...
Check binds. Print exp. Locate binds.
...
| t_receive : forall G k P D S T x,
    oft (add x S G) P (add k T D) ->
    oft G (receive k x P) (add k (input S T) D)

| t_throw : forall G (k k' : atom) P D T T',
    oft G P (add k T' D) ->
    oft G (throw k k' P) (add k (ch_output T T') (add k' T D))

| t_catch : forall (L : seq atom) G k P D T T' x,
    oft G P (add x T (add k T' D)) ->
    oft G (catch k x P) (add k (ch_input T T') D)
...
| t_nu_ch : forall (L : seq atom) G P D x,
    oft G P (add x bot D) ->
    oft G (nu_ch x P) D
